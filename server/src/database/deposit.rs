...
pub async fn get_token_status(pool: &sqlx::PgPool, token_id: &str) -> Option<bool> {
    let row = sqlx::query("SELECT confirmed, spent FROM public.tokens WHERE token_id = $1")
        .bind(token_id)
        .fetch_one(pool)
        .await;
...
pub async fn set_token_spent(pool: &sqlx::PgPool, token_id: &str) {
    ...
    let query = "UPDATE tokens SET spent = true WHERE token_id = $1";
...
pub async fn check_existing_key(pool: &sqlx::PgPool, auth_key: &XOnlyPublicKey) -> bool {
    let row = sqlx::query("SELECT 1 FROM statechain_data WHERE auth_xonly_public_key = $1")
