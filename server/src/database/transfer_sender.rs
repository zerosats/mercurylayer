...
pub async fn exists_msg_for_same_statechain_id_and_new_user_auth_key(
    ...
    let query = "\
        SELECT COUNT(*) FROM statechain_transfer WHERE new_user_auth_public_key = $1 AND \
                 statechain_id = $2 AND batch_id = $3"
        .to_string();
...
pub async fn get_batch_time_by_batch_id(
    ...
    let query = "\
        SELECT batch_time FROM statechain_transfer WHERE batch_id = $1
        AND locked = true";
...
pub async fn insert_new_transfer(
    ...
    let query2 = if batch_id.is_none() {
        "INSERT INTO statechain_transfer (statechain_id, new_user_auth_public_key, x1, locked, \
         locked2) VALUES ($1, $2, $3, $4, $5)"
    } else {
        "INSERT INTO statechain_transfer (statechain_id, new_user_auth_public_key, x1, batch_id, \
         batch_time, locked, locked2) VALUES ($1, $2, $3, $4, $5, $6, $7)"
    };
...
pub async fn update_transfer_msg(
    ...
    let query = "\
        UPDATE statechain_transfer SET encrypted_transfer_msg = $1, updated_at = NOW() WHERE \
                 statechain_id = $2 AND new_user_auth_public_key = $3 AND updated_at = (SELECT \
                 MAX(updated_at) FROM statechain_transfer WHERE statechain_id = $2)";
