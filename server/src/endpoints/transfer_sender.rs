...
pub enum BatchTransferValidationResult {
    ...
    /// or the batch is complete or expired and the batch_id is different from
    /// the new_batch_id (or null)
    Success,
}
...
pub async fn validate_batch_transfer(
    ...
    // get an extistent batch according to the statecoin, in case the user sent a
    // repeated statecoin
    ...
    if batch_info.is_some() {
        ...
        if !is_batch_expired(batch_time) {
            ...
            if new_batch_id.is_some() && *new_batch_id.as_ref().unwrap() == batch_id {
    ...
    // then we check if the user sends a existing batch_id, trying to add a new
...
pub async fn transfer_sender(
    ...
    .await
    {
        let message = if batch_id.is_some() {
            "Transfer message already exists for this statechain_id, new_user_auth_key and \
             batch_id."
        } else {
