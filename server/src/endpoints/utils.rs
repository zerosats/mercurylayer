...
pub async fn get_auth_key_by_statechain_id(
    ...
    let row = sqlx::query(
        "SELECT auth_xonly_public_key FROM public.statechain_data WHERE statechain_id = $1",
    )
    .bind(statechain_id)
    .fetch_one(pool)
    .await;
...
pub async fn validate_signature_given_public_key(
    ...
    secp.verify_schnorr(&signed_message, &msg, &auth_key)
        .is_ok()
}
...
pub async fn info_keylist(
    ...
    let query_sigs = "\
    SELECT tx_n, statechain_id, created_at::TEXT FROM statechain_signature_data";
