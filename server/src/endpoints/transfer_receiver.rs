...
use secp256k1_zkp::{Message, PublicKey, Secp256k1, schnorr::Signature};
...
pub async fn statechain_info(
    ...
    let x1_pubkey =
        crate::database::transfer_receiver::get_x1pub(&statechain_entity.pool, statechain_id).await;
...
pub enum BatchTransferReceiveValidationResult {
    /// The statecoin batch is locked (not expired yet and not all coins are
    /// unlocked)
    StatecoinBatchLockedError(String),
    /// The batch_id sent by the user is expired
    ExpiredBatchTimeError(String),
    /// Success means there is no batch_id for the statecoin or all the coins of
    /// the batch are unlocked.
    Success,
}
...
pub async fn validate_batch(
    ...
    if batch_info.is_some() {
        ...
        if is_batch_expired(batch_time) {
            // the batch time has not expired. It is possible to add a new coin to the
            // batch.
...
pub async fn transfer_receiver(
    ...
    // TODO: check if the statechain_id is within a batch and if it is, check if the
    // batch is still open or expired. If open, check all coins are unlocked. If
    // not, return 400 error. If expired, return 400 error.
    ...
    if !secp
        .verify_schnorr(&signed_message, &msg, &auth_pubkey)
        .is_ok()
    {
        let response_body = json!({
            "message": "Signature does not match authentication key."
