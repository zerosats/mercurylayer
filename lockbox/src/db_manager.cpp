...
namespace db_manager {
    ...
    void ensure_generated_public_key_schema(pqxx::connection& conn) {
        ...
        txn.exec("ALTER TABLE generated_public_key ADD COLUMN IF NOT EXISTS status varchar(32) NOT NULL DEFAULT 'active';");
        txn.exec("ALTER TABLE generated_public_key ADD COLUMN IF NOT EXISTS split_id varchar(50);");
        txn.exec("CREATE INDEX IF NOT EXISTS generated_public_key_split_id_idx ON generated_public_key(split_id);");
        txn.commit();
    }
    ...
    bool save_generated_public_key(
        ...
        try
        {
            ...
            if (conn.is_open()) {
                ...
                ensure_generated_public_key_schema(conn);
                ...
                std::string insert_query =
                    "INSERT INTO generated_public_key (sealed_keypair, public_key, statechain_id, status) VALUES ($1, $2, $3, 'active');";
                pqxx::work txn2(conn);
        ...
        try
        {
            ...
            if (conn.is_open()) {
                ...
                std::string sealed_keypair_query =
                    "SELECT sealed_keypair, sealed_secnonce, public_nonce FROM generated_public_key WHERE statechain_id = $1 AND status IN ('active', 'pending_split');";
    ...
    bool save_pending_split_child_key(
        ...
        try {
            ...
            pqxx::work txn(conn);
            txn.exec_params(
                "INSERT INTO generated_public_key (sealed_keypair, public_key, statechain_id, status, split_id) "
                "VALUES ($1, $2, $3, 'pending_split', $4) "
                "ON CONFLICT (public_key) DO NOTHING;",
                sealed_data_view,
                public_key_data_view,
                statechain_id,
                split_id);
            txn.commit();
    ...
    bool get_split_child_public_keys(
        ...
        try {
            ...
            pqxx::nontransaction ntxn(conn);
            auto result = ntxn.exec_params(
                "SELECT statechain_id, public_key FROM generated_public_key WHERE split_id = $1 AND status = 'pending_split' ORDER BY id ASC;",
                split_id);

            for (auto row : result) {
                auto public_key_view = row["public_key"].as<std::basic_string<std::byte>>();
                std::string statechain_id = row["statechain_id"].as<std::string>();
                std::string public_key = utils::key_to_string(reinterpret_cast<const unsigned char*>(public_key_view.data()), public_key_view.size());
                child_public_keys.push_back(std::make_pair(statechain_id, public_key));
            }
    ...
    bool finalize_split_keys(
        ...
        try {
            ...
            pqxx::work txn(conn);
            txn.exec_params(
                "UPDATE generated_public_key SET status = 'active' WHERE split_id = $1 AND status = 'pending_split';",
                split_id);
            txn.exec_params(
                "UPDATE generated_public_key SET sealed_keypair = NULL, sealed_secnonce = NULL, public_nonce = NULL, status = 'split' WHERE statechain_id = $1;",
    ...
    bool abort_split_keys(
        ...
        try {
            ...
            pqxx::work txn(conn);
            txn.exec_params(
                "DELETE FROM generated_public_key WHERE split_id = $1 AND status = 'pending_split';",
                split_id);
            txn.exec_params(
                "UPDATE generated_public_key SET status = 'active' WHERE statechain_id = $1 AND status = 'pending_split';",
                parent_statechain_id);
            txn.commit();
