        ...
        for enc_message in values {
            ...
            if coin.is_some() {
                ...
                if is_msg_valid.is_err() {
                    println!("Validation error: {}", is_msg_valid.err().unwrap());
                    continue;
                }
                ...
                if message_result.is_err() {
                    println!("Processing error: {}", message_result.err().unwrap());
                    continue;
                }
                ...
                if !message_result.duplicated_coins.is_empty() {
                ...
                if new_coin.is_err() {
                    println!("Error: {}", new_coin.err().unwrap());
                    continue;
                }
...
async fn get_msg_addr(auth_pubkey: &str, client_config: &ClientConfig) -> Result<Vec<String>> {
    let path = format!("transfer/get_msg_addr/{}", auth_pubkey);
...
pub fn split_backup_transactions(backup_transactions: &Vec<BackupTx>) -> Vec<Vec<BackupTx>> {
    ...
    for tx in backup_transactions {
        ...
        grouped_txs.entry(key).or_default().push(tx.clone());
    }
...
async fn process_encrypted_message(
    ...
    for (index, backup_transactions) in grouped_backup_transactions.iter().enumerate() {
        if index == 0 {
            ...
            // the pub_auth_key has not been updated yet in the server (it will be updated
            // after the transfer/receive call) So we need to manually sign the
            // statechain_id with the client_auth_key
...
async fn send_transfer_receiver_request_payload(
    ...
    if status == StatusCode::OK {
        let response: mercurylib::transfer::receiver::TransferReceiverPostResponsePayload =
            serde_json::from_str(value.as_str())?;
        Ok(TransferReceiveRequestResult {
            is_batch_locked: false,
        ...
        Err(anyhow::anyhow!(
            "{}: {}",
            "Failed to update transfer message",
            value
        ))
    }
}
