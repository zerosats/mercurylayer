...
pub async fn sign_first(
    ...
    // let value =
    // request.json(&sign_first_request_payload).send().await?.text().await?;
    ...
    if status != StatusCode::OK {
        ...
        return Err(anyhow::anyhow!(
            "{}",
            error_message["message"].as_str().unwrap()
        ));
    }

    let sign_first_response_payload: mercurylib::transaction::SignFirstResponsePayload =
...
pub async fn sign_second(
    client_config: &ClientConfig,
    partial_sig_request: &PartialSignatureRequestPayload,
) -> Result<MusigPartialSignature> {
    let endpoint = client_config.statechain_entity.clone();
