...
async fn check_deposit(
    ...
    if utxo.is_none() {
        return Ok(None);
        // return Err(anyhow!("There is no UTXO with the address {} and the
        // amount {}", coin.aggregated_address.as_ref().unwrap(),
        // coin.amount.unwrap()));
    }
    ...
    if coin.status == CoinStatus::INITIALISED {
        ...
        let activity_utxo = format!("{}:{}", utxo.tx_hash, utxo.tx_pos);
...
async fn check_transfer(client_config: &ClientConfig, coin: &Coin) -> Result<bool> {
    ...
    // if the enclave's public key is no longer part of the coin, the coin has been
    // transferred
...
async fn check_withdrawal(client_config: &ClientConfig, coin: &mut Coin) -> Result<()> {
    ...
    if utxo.is_none() {
        // sometimes the transaction has not yet been transmitted to the specified
        // Electrum server return Err(anyhow!("There is no UTXO with the address
        // {} and the txid {}", coin.withdrawal_address.as_ref().unwrap(), txid));
        return Ok(());
    }
