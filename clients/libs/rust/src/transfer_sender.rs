...
use crate::{
    ...
    deposit::create_tx1,
    sqlite_manager::{get_backup_txs, get_wallet, update_backup_txs, update_wallet},
...
pub async fn create_backup_transactions(
    ...
    // throw error if duplicated_indexes contains an index that does not exist in
    // wallet.coins this can be moved to the caller function
    ...
    let backup_transactions =
        get_backup_txs(&client_config.pool, &wallet.name, statechain_id).await?;

    // Get coins that already have a backup transaction
    for coin in wallet.coins.iter_mut() {
        // Check if coin matches any backup transaction and has one of the specified
        // statuses
        ...
        if duplicated_indexes.is_some() {
            ...
            if coin.statechain_id == Some(statechain_id.to_string())
                && coin.status == CoinStatus::DUPLICATED
                && duplicated_indexes.is_some()
                && duplicated_indexes
                    .as_ref()
                    .unwrap()
                    .contains(&coin.duplicate_index)
            {
                coin_to_add = true;
            }
        }
        ...
        if has_matching_tx || coin_to_add {
            ...
            if current_blockheight > coin.locktime.unwrap() {
                return Err(anyhow::anyhow!(
                    "The coin is expired. Coin locktime is {} and current blockheight is {}",
                    coin.locktime.unwrap(),
                    current_blockheight
                ));
            }
    ...
    // The backup transaction for the CONFIRMED coin is created when it is detected
    // in the mempool So it is exepcted that the coin with duplicate_index == 0
    // is in the list since it must have at least one backup transaction
    ...
    if coins_with_zero_index.len() != 1 {
        return Err(anyhow!(
            "There must be at least one coin with duplicate_index == 0"
        ));
    }
        ...
        if coin.status == CoinStatus::DUPLICATED {
            ...
            for unspent in utxo_list {
                    ...
                    && coin.utxo_vout == Some(unspent.tx_pos as u32)
                {
                    ...
                    if !is_confirmed {
                        return Err(anyhow!(
                            "The coin with duplicated index {} has not yet been confirmed. This \
    ...
    coin_list.sort_by(|a, b| match (&a.status, &b.status) {
    ...
    for coin in coin_list {
        ...
        filtered_transactions.sort_by_key(|a| a.tx_n);
        ...
        let signed_tx = create_backup_tx_to_receiver(
            client_config,
            coin,
            bkp_tx1,
            recipient_address,
    ...
    new_backup_transactions.sort_by_key(|a| a.tx_n);
...
pub async fn execute(
    ...
    let is_coin_duplicated = wallet.coins.iter().any(|c| {
        c.statechain_id == Some(statechain_id.to_string()) && c.status == CoinStatus::DUPLICATED
    });

    if is_coin_duplicated && !force_send {
        return Err(anyhow::anyhow!(
            "Coin is duplicated. If you want to proceed, use the command '--force, -f' option. \
             You will no longer be able to move other duplicate coins with the same statechain_id \
             and this will cause PERMANENT LOSS of these duplicate coin funds."
        ));
    }
    ...
    if are_there_duplicate_coins_withdrawn {
        return Err(anyhow::anyhow!(
            "There have been withdrawals of other coins with this same statechain_id (possibly \
             duplicates).This transfer cannot be performed because the recipient would reject it \
             due to the difference in signature count.This coin can be withdrawn, however."
        ));
    }

    let coin = &wallet.coins
        .iter()
        .filter(|c| c.statechain_id == Some(statechain_id.to_string()) && (c.status == CoinStatus::CONFIRMED || c.status == CoinStatus::IN_TRANSFER) && c.duplicate_index == 0) // Filter coins with the specified statechain_id
        .min_by_key(|c| c.locktime.unwrap_or(u32::MAX)); // Find the one with the lowest locktime

    if coin.is_none() {
        return Err(anyhow!(
            "No coins with status CONFIRMED or IN_TRANSFER associated with this statechain ID \
             were found"
        ));
    }
    ...
    let backup_transactions = create_backup_transactions(
        client_config,
        recipient_address,
        &mut wallet,
        &statechain_id,
    ...
    let transfer_update_msg_request_payload = create_transfer_update_msg(
        &x1,
        recipient_address,
        &coin,
        &transfer_signature,
    ...
    let status = request
        .json(&transfer_update_msg_request_payload)
        .send()
        .await?
        .status();
