    ...
    assert!(
        error_msg
            == "Coin is duplicated. If you want to proceed, use the command '--force, -f' option. \
                You will no longer be able to move other duplicate coins with the same \
                statechain_id and this will cause PERMANENT LOSS of these duplicate coin funds."
    );
    ...
    assert!(
        error_msg
            == "There have been withdrawals of other coins with this same statechain_id (possibly \
                duplicates).This transfer cannot be performed because the recipient would reject \
                it due to the difference in signature count.This coin can be withdrawn, however."
    );
