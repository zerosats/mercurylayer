...
async fn tm01(
    ...
    let new_coin = wallet1
        .coins
        .iter()
        .find(|&coin| coin.aggregated_address == Some(address.clone()))
        .unwrap();
    ...
    // this first "double spend" is legitimate, as it will overwrite the previous
    // transaction
    let result = mercuryrustlib::transfer_sender::execute(
        client_config,
    ...
    assert!(received_statechain_ids[0] == *statechain_id);

    let batch_id = None;

    // this second "double spend" is not legitimate, as the statecoin has already
    // been received by wallet3
    let result = mercuryrustlib::transfer_sender::execute(
        client_config,
        &wallet2_transfer_adress,
        &wallet1.name,
    ...
    assert!(
        result
            ...
            .unwrap()
            .to_string()
            .contains("Signature does not match authentication key")
    );

    // If we update wallet1, the error will happen when we try to send the coin to
    // wallet2 The step above tested that the sender can double spend the coin,
    // but the server will not accept it
    ...
    assert!(result.err().unwrap().to_string().contains(
        "No coins with status CONFIRMED or IN_TRANSFER associated with this statechain ID were \
         found"
    ));
