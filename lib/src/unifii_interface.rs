...
// They were created because we would need to expose to the unifii library some
// low level functions and struct that are not part of the wallet API
...
use crate::{
    MercuryError, decode_transfer_address,
    transfer::{
...
#[cfg_attr(feature = "bindings", uniffi::export)]
pub fn fii_decrypt_transfer_msg(
    encrypted_message: &str,
    private_key_wif: &str,
) -> Result<FFITransferMsg, MercuryError> {
    let transfer_msg =
        crate::transfer::receiver::decrypt_transfer_msg(encrypted_message, private_key_wif)?;
    Ok(transfer_to_ffi_msg(transfer_msg))
}

#[cfg_attr(feature = "bindings", uniffi::export)]
pub fn ffi_verify_transfer_signature(
    new_user_pubkey: &str,
    ...
    crate::transfer::receiver::verify_transfer_signature(
        new_user_pubkey,
        tx0_outpoint,
        &transfer_msg,
    )
...
#[cfg_attr(feature = "bindings", uniffi::export)]
pub fn fii_validate_tx0_output_pubkey(
    ...
    let transfer_msg = ffi_to_transfer_msg(ffi_transfer_msg.clone())?;
    crate::transfer::receiver::validate_tx0_output_pubkey(
        enclave_public_key,
        &transfer_msg,
        tx0_outpoint,
        tx0_hex,
...
#[cfg_attr(feature = "bindings", uniffi::export)]
pub fn fii_verify_latest_backup_tx_pays_to_user_pubkey(
    ffi_transfer_msg: &FFITransferMsg,
    client_pubkey_share: &str,
    network: &str,
) -> Result<bool, MercuryError> {
    let transfer_msg = ffi_to_transfer_msg(ffi_transfer_msg.clone())?;
    crate::transfer::receiver::verify_latest_backup_tx_pays_to_user_pubkey(
        &transfer_msg,
        client_pubkey_share,
        network,
    )
...
#[cfg_attr(feature = "bindings", uniffi::export)]
pub fn fii_create_transfer_receiver_request_payload(
    statechain_info: &StatechainInfoResponsePayload,
    ffi_transfer_msg: &FFITransferMsg,
    coin: &Coin,
) -> Result<TransferReceiverRequestPayload, MercuryError> {
    let transfer_msg = ffi_to_transfer_msg(ffi_transfer_msg.clone())?;
    crate::transfer::receiver::create_transfer_receiver_request_payload(
        statechain_info,
        &transfer_msg,
        coin,
    )
...
//
// let transfer_msg = ffi_to_transfer_msg(ffi_transfer_msg.clone())?;
// crate::transfer::receiver::validate_signature_scheme(&transfer_msg,
// &statechain_info, tx0_hex, current_blockheight, fee_rate_tolerance,
// current_fee_rate_sats_per_byte, lockheight_init, interval) }
