...
use secp256k1_zkp::{PublicKey, Secp256k1, SecretKey, ffi::types::AlignedType};
...
impl Wallet {
    ...
    pub fn generate_new_key(
        ...
        let secret_key = child
            ...
            .private_key;
        let public_key: secp256k1_zkp::PublicKey = xpub
            .derive_pub(&secp, &[change_index_number, address_index_number])?
            .public_key;
    ...
    pub fn get_new_coin(&self) -> Result<Coin, MercuryError> {
        ...
        let client_secret_key_wif =
            PrivateKey::from_slice(&client_secret_key.secret_bytes(), network)?.to_wif();
        let auth_secret_wif =
            PrivateKey::from_slice(&auth_secret.secret_bytes(), network)?.to_wif();
        ...
        let coin_address =
            encode_sc_address(&client_pubkey_share, &auth_key_data.public_key, network)?;
